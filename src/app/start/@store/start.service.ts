...
export class StartService {
  ...
  resetSequence(races: Race[], sequence: StartFlagSequence) {
    ...
    this.startStore.update({ races, sequence });
  }
  ...
  public runStartSequence(option: 'Now' | 'NextMinute' | 'ScheduledTime') {
    ...
    this.startStore.update(s => {
      ...
      return { firstStartTime: firstStartTime,
               state: StartStatus.running,
               races: races,
               flagTimes: flags };
    });
  }
  ...
  private _calculateFlags(races: Race[], sequence: StartFlagSequence): StartFlagTiming[] {
    ...
    for (const race of races) {
      ...
      const prepUpTime = subMinutes(startTime, sequence.prepUp);
      ...
      const prepDownTime = subMinutes(startTime, sequence.prepDown);
  ...
  private _findOrCreateFlagForTime(time: Date, flags: StartFlagTiming[]): StartFlagTiming {
    let flag = flags.find(f => isEqual(f.time, time));
    if (!flag) {
      flag = createStartFlagTiming({ time });
      flags.push(flag);
    }
    return flag;
  }
